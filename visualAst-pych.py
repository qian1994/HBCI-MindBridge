...
win = visual.Window(size=(0, 0), fullscr=False, allowGUI=False)
...
class Paradigms(QWidget, Ui_Paradigms):
    ...
    def doTimerTimeout(self): #定时中断相应
        ...
        while clock.getTime() < 5:
            t = clock.getTime()
            # print(t,  t % (1 / frequency), 1 / (2 * frequency))

            if t % (1 / frequency) < (1 / (2 * frequency)):
                print(self.count, 'asdfasdfasdf')
                if self.count == 0:
                    self.trigger(-1)
    ...
    def showFlash(self):
        ...
        # self.doTimerTimeout()
        # start1 = Thread(target=self.doTimerTimeout)
        # start1.start()
        self.doTimerTimeout()
